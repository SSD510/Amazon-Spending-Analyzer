...
if uploaded_file:
    ...
    # Fix: incorrect string replace syntax
    df.columns = [c.strip().lower().replace(" ", "_") for c in df.columns]

    price_col = None

    # Fix: syntax errors in condition
    for col in df.columns:
        if ("total" in col) or ("charged" in col) or ("price" in col):
            price_col = col
            break
    ...
    if price_col is None:
        ...
        df[price_col] = (
            df[price_col]
            .replace(r"[\$,]", "", regex=True)  # Fix: proper escaping & regex
            .astype(float)
        )
        ...
        if "category" in df.columns:
            # Fix: syntax issues + missing brackets
